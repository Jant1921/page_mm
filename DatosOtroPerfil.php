...
if (isset($_POST['persona1'])) {
	$id_visit=$person_suger[$posicion];
}elseif (isset($_POST['persona2'])) {
	$id_visit=$person_suger[$posicion+1];
}elseif (isset($_POST['persona3'])) {
	$id_visit=$person_suger[$posicion+2];
}elseif (isset($_POST['persona4'])) {
	$id_visit=$person_suger[$posicion+3];
}elseif (isset($_POST['persona5'])) {
	$id_visit=$person_suger[$posicion+4];
}elseif (isset($_POST['persona6'])) {
	$id_visit=$person_suger[$posicion+5];
}
...
oci_bind_by_name($stid,':resultado',$visit_nombre,30);
oci_bind_by_name($stid,':id_persona',$id_visit);
oci_execute($stid);
$_SESSION['signed_nombre']=$visit_nombre; // Initializing Session
$stid = oci_parse($conn, 'begin :resultado := get_persona_edad(:per_id);end;');
oci_bind_by_name($stid,':resultado',$person_edad,10);
oci_bind_by_name($stid,':per_id',$id_visit);
oci_execute($stid);
...
oci_bind_by_name($stid,':person_id',$id_visit);
oci_execute($stid);
...
oci_bind_by_name($stid,':idPersona',$id_visit);
oci_execute($stid);
// saber si fuma
...
oci_bind_by_name($stid,':resultado',$person_profesion,30);
